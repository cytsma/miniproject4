...
```{r}
uber_jfk_jan <- read.csv("./jan2015uber.csv")

uber_jfk_jan <- uber_jfk_jan %>%
...
ggplot(uber_jfk_jan, aes(x = Date, y = rides_per_day)) +
  geom_point()
uber_jfk_jan #can also print the data frame later if necessary
```

```{r}
uber_ewr_jan <- read.csv("./jan2015uber.csv")

uber_ewr_jan <- uber_ewr_jan %>%
  filter(locationID == 1) %>%
  group_by(Date) %>%
  summarise(rides_per_day = n())
...
ggplot(uber_ewr_jan, aes(x = Date, y = rides_per_day)) +
  geom_point()
uber_ewr_jan #can also print the data frame later if necessary
